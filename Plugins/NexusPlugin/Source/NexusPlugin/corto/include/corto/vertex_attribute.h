...
public:
	...
	virtual void quantize(uint32_t nvert, const char *bbuffer) {
		uint32_t n = N*nvert;
		...
		case INT32:
			for(uint32_t i = 0; i < n; i++)
				values[i] = ((const int32_t *)bbuffer)[i]/q;
			break;
		case INT16:
			for(uint32_t i = 0; i < n; i++)
				values[i] = ((const int16_t *)bbuffer)[i]/q;
			break;
		case INT8:
			for(uint32_t i = 0; i < n; i++)
				values[i] = ((const int8_t *)bbuffer)[i]/q;
			break;
		case FLOAT:
			for(uint32_t i = 0; i < n; i++)
				values[i] = ((const float *)bbuffer)[i]/q;
			break;
		case DOUBLE:
			for(uint32_t i = 0; i < n; i++)
				values[i] = ((const double *)bbuffer)[i]/q;
			break;
...
//			cout << "max: " << max << " " << " bits: " << bits << endl;
	...
	virtual void deltaEncode(std::vector<Quad> &context) {
		...
		for(uint32_t i = 1; i < context.size(); i++) {
			Quad &qq = context[i];
			if(qq.a != qq.b && (strategy & PARALLEL)) {
				for(int c = 0; c < N; c++)
					diffs[i*N + c] = values[qq.t*N + c] - (values[qq.a*N + c] + values[qq.b*N + c] - values[qq.c*N + c]);
			} else {
				for(int c = 0; c < N; c++)
					diffs[i*N + c] = values[qq.t*N + c] - values[qq.a*N + c];
			}
		}
